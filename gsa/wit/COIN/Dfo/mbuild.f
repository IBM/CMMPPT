...
*
      ...
      IF ( LENW .LT. 1 .AND. FROBEN ) THEN
         IF ( IPRINT .GE. 0 ) WRITE( IOUT, 1100 ) -LENW+1
         STOP
      ENDIF

      IF ( LIWRK .LT. 1 .AND. FROBEN ) THEN
...
*
      ...
      INTEGER          N, Q, LPNT, BASE, IWRK(LWRK), LWRK, LIWRK, NEQCON

      DOUBLE PRECISION G(N), H(N,N), KAPPA, PNTINT(LPNT), VALINT(Q),
     *                 MATR(Q,Q)   , WRK(LIWRK), Y(LPNT-N), VY(Q-1)
      LOGICAL          FAIL

      DOUBLE PRECISION DDOT, F06QGF

      EXTERNAL         DDOT, F06QGF
...
*
      ...
      DOUBLE PRECISION ANORM, RCOND, SUM
