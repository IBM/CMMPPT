...
C*******************************************************************************
C
C    $Id: update_b.f,v 1.1 2002/05/02 18:52:20 andreasw Exp $
C
C-------------------------------------------------------------------------------
C                                 Title
C-------------------------------------------------------------------------------
...
CTODO Check if xorigtmp is reallty necessary
            ...
            call GET_G(N, RW(p_pert+1), IVAR, NORIG, RW(p_xorig+1),
     1           RW(p_gdiff+1), LRW-p_rwend, RW(p_rwend+1), IERR)
            if( IERR.gt.0 ) then
               write(line,*)
     1              'update_b: Error: get_g ends with IERR = ',IERR
               call C_OUT(2,0,1,line)
               goto 9999
            elseif( IERR.ne.0 ) then
               write(line,*)
     1              'update_b: Warning: get_g ends with IERR = ',IERR
               call C_OUT(2,0,1,line)
            endif
            p_rwend = p_pert
C
...
C
C     Do Powell damping if desired (s. pg. 540 Nocedal's book)
C
