...
C*******************************************************************************
C
      subroutine INIT_BND(NORIG, N, NIND, M, NFIX, IFIX,
     1                    XORIG, X, NLBO, ILBO, BNDS_LO, V_LO,
     1                    NLB, ILB, BNDS_L, NUBO, IUBO, BNDS_UO,
     1                    V_UO, NUB, IUB, BNDS_U, IVAR,
     1                    LRW, RW, LIW, IW, IERR)
C
C*******************************************************************************
C
C    $Id: init_bnd.f,v 1.2 2002/05/08 18:17:29 andreasw Exp $
C
C-------------------------------------------------------------------------------
C                                 Title
C-------------------------------------------------------------------------------
...
C     and BNDS_U (only required for warm start option IMUINIT = 0)
C
      if( QINIT.eq.0 ) then
         p_vcopy = p_rwend
         ...
         do i = 1, NLB
            V_LO(i) = RW(p_vcopy+IW(p_ilbo1+IVAR(ILB(i))))
         enddo
         call DCOPY(NUBO, V_UO, 1, RW(p_vcopy+1), 1)
         do i = 1, NUB
            V_UO(i) = RW(p_vcopy+IW(p_iubo1+IVAR(IUB(i))))
         enddo
         p_rwend = p_vcopy
      endif
C
