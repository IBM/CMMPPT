...
C
     ...
     3                     LSLACKS, YPY, THETA1, THETA2, ALPHA,
...
C*******************************************************************************
C
C    $Id: new_basis.f,v 1.2 2002/11/24 21:45:43 andreasw Exp $
C
C-------------------------------------------------------------------------------
C                                 Title
C-------------------------------------------------------------------------------
...
C
     ...
     1               RW(p_xtmp+1))
C
C     if necessary, reorder LAM and LAMOLD (for aug_lag and LSLACKS)
C
      if( LSLACKS .and. QMERIT.eq.3 ) then
         call REORDER_X(N, NORIG, LAM(M+1), IW(p_ivarold+1), IVAR,
     1        RW(p_xtmp+1))
         call REORDER_X(N, NORIG, LAMOLD(M+1), IW(p_ivarold+1), IVAR,
     1        RW(p_xtmp+1))
      endif
C
      p_rwend = p_xtmp
...
C
     ...
     2            S_L, S_U, C, LSLACKS, 0,
     2            RW(p_ypydummy+1), THETA1, THETA2, newbastmp,
...
C
     ...
     2            S_L, S_U, C, LSLACKS, 0, YPY,
     2            THETA1, THETA2, newbastmp, condctmp,
     1            KCONSTR, LRS, RS, LIS, IS,
     2            LRW-p_rwend, RW(p_rwend+1),
     4            LIW-p_iwend, IW(p_iwend+1), IERR)
      if( IERR.gt.0 ) then
