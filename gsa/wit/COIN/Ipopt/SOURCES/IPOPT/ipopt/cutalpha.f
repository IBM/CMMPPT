...
C
     ...
     2                    LSLACKS, ALPHA, ALPHA_DUAL, CALPHA)
C
C*******************************************************************************
C
C    $Id: cutalpha.f,v 1.1 2002/05/02 18:52:17 andreasw Exp $
C
C-------------------------------------------------------------------------------
C                                 Title
C-------------------------------------------------------------------------------
...
C
      ...
      if( .not.LSLACKS ) then
         ...
         do i = 1, NLB
            dxi = DX(ILB(i)) + X(ILB(i)) - S_L(i) - BNDS_L(i)
            if( dxi.lt.0.d0 ) then
               alphap = dmin1(alphap, TAU*(-S_L(i)/dxi) )
            endif
         enddo
         do i = 1, NUB
            dxi = DX(IUB(i)) + X(IUB(i)) + S_U(i) - BNDS_U(i)
            if( dxi.gt.0.d0 ) then
               alphap = dmin1(alphap, TAU*S_U(i)/dxi )
            endif
         enddo
