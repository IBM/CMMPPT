...
C
     ...
     2     NLB, ILB, NUB, IUB, LSLACKS, S_L, S_U, BNDS_L, BNDS_U,
     3     SIGMA_L, SIGMA_U, YPY, REGU, SOC_FLAG, RESTO,
     3     KCONSTR, LRS, RS, LIS, IS,
     4     LRW, RW, LIW, IW, IERR)
C
C*******************************************************************************
C
C    $Id: get_pz.f,v 1.1 2002/05/02 18:52:17 andreasw Exp $
C
C-------------------------------------------------------------------------------
C                                 Title
C-------------------------------------------------------------------------------
...
C
      ...
      integer p_corr, p_icorr, p_a, p_ipiv, p_w, p_iperm, p_apf
      integer p_ztz, idummy, p_sigma, p_bpz, p_s, p_x, p_iwork, p_bzg
      integer info, i, k, j
      double precision lam_b, mineig, rcond, ferr, berr
      double precision eigmin, eigmax, eigold, val1, val2, regu1, diff
      double precision zeta, gzbw, gzbzg
      character*80 line(4), equed
C
...
C
      if( abs(QQUASI).eq.4 .and..not.RESTO ) then
         do i = 1, IEIGS
            k = IW(p_icorr+i)
...
C
      ...
      if( QDAMP.ne.0 .and. .not.RESTO ) then
         p_bzg   = p_rwend
         p_rwend = p_bzg + NIND
         if( p_rwend.gt.LRW ) then
            IERR = 98
...
C
     ...
     2              LSLACKS, S_L, S_U, BNDS_L, BNDS_U,
