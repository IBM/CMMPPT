...
******************************************************************************
      ...
      if( LSLACKS ) then
         ...
         enddo
         enrm0 = enrm0 + DDOT(N, RW(p_tmp+1), 1, RW(p_tmp+1), 1)
         u0c0  = u0c0  + DDOT(N, LAMOLD(M+1), 1, RW(p_tmp+1), 1)
         ugvh0 = ugvh0 + DDOT(N, LAM(M+1)   , 1, RW(p_tmp+1), 1)
         p_rwend = p_tmp
      endif
...
C
         if( .not.LSLACKS ) then
            do i = 1, NLB
               RW(p_slnew+i) =
CWEG     1              dmax1(macheps, RW(p_xnew+ILB(i)) - BNDS_L(i))
     1              dmax1(machtiny, RW(p_xnew+ILB(i)) - BNDS_L(i))
            enddo
            do i = 1, NUB
               RW(p_sunew+i) =
CWEG     1              dmax1(macheps, BNDS_U(i) - RW(p_xnew+IUB(i)))
     1              dmax1(machtiny, BNDS_U(i) - RW(p_xnew+IUB(i)))
            enddo
         else
            do i = 1, NLB
               k = ILB(i)
               RW(p_slnew+i) = dmax1(machtiny,
     1                    S_L(i) + ALPHA*( DX(k)+X(k)-S_L(i)-BNDS_L(i)))
            enddo
            do i = 1, NUB
               k = IUB(i)
               RW(p_sunew+i) = dmax1(machtiny,
...
C
     ...
     1        RW(p_lamnew+1), 1)
         if( LSLACKS ) then
            call DCOPY(N, LAM(M+1), 1, RW(p_lamnew+M+1), 1)
            call DSCAL(N, ALPHA, RW(p_lamnew+M+1), 1)
            call DAXPY(N, (1.d0-ALPHA), LAMOLD(M+1), 1,
     1           RW(p_lamnew+M+1), 1)
         endif
...
*   UGVH  = U(X)*C(X) - U(X0)*C(X0)   = U*C - U0*C0
         ...
         if( LSLACKS ) then
            ...
            enddo
            enrm = enrm + DDOT(N, RW(p_tmp+1), 1, RW(p_tmp+1), 1)
            uc   = uc   + DDOT(N, RW(p_lamnew+M+1), 1, RW(p_tmp+1), 1)
            p_rwend = p_tmp
         endif
         ugvh = uc - u0c0
