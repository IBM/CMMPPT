...
// Default Constructor
SbbHeuristic::SbbHeuristic() 
  :model_(NULL)
{
}

// Constructor from model
SbbHeuristic::SbbHeuristic(SbbModel & model)
:
  model_(&model)
{
}
...
// update model
void SbbHeuristic::setModel(SbbModel * model)
{
  model_ = model;
}

// Default Constructor
SbbRounding::SbbRounding() 
  :SbbHeuristic()
{
...
// Constructor from model
SbbRounding::SbbRounding(SbbModel & model)
  :SbbHeuristic(model)
{
...
// Clone
SbbHeuristic *
SbbRounding::clone() const
{
  return new SbbRounding(*this);
}

// Copy constructor 
SbbRounding::SbbRounding(const SbbRounding & rhs)
:
  SbbHeuristic(rhs),
  matrix_(rhs.matrix_),
...
int
SbbRounding::solution(double & solutionValue,
			 double * betterSolution)
{
  ...
  double integerTolerance = model_->getDblParam(SbbModel::SbbIntegerTolerance);
  ...
  for (i=0;i<numberRows;i++) {
	...
	if (columnLength[iColumn]==1) {
	  ...
	  if (thisInfeasibility*elementValue>0.0) {
		...
		assert (currentValue-distance>=lowerValue-primalTolerance);
  ...
  if (!penalty) {
	...
	printf("Debug SbbRounding giving bad solution\n");
      }
    }
  }
  delete [] newSolution;
...
// update model
void SbbRounding::setModel(SbbModel * model)
{
  model_ = model;
  // Get a copy of original matrix (and by row for rounding);
