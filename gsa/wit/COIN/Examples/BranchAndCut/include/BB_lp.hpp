...
class BB_lp : public BCP_lp_user {
   BCP_parameter_set<BB_lp_par> par;
   // necessary problem data
   int indrows;
   double* rlb;
   double* rub;
   CoinPackedMatrix indexed;

   // A placeholder for the indices of the cuts violated in test feasibilty
   // (we have to test the cuts, so why test them once again in
   // generate_cuts_in_lp() ?)
   std::vector<int> violated_cuts;
   // a fixed length array, used to compute the lhs for the indexed cuts
   double* lhs;
