...
void
MC_generate_SDP_cuts(const MC_problem& mc, const double* x,
		     const int maxnewcuts, const double minviol,
		     BCP_vec<BCP_cut*>& new_cuts,
		     BCP_vec<BCP_row*>& new_rows)
{
   int i, j, k;
   const int n = mc.num_nodes;
   NEWMAT::SymmetricMatrix S(n);
   NEWMAT::Matrix V(n,n);
   NEWMAT::DiagonalMatrix D(n);
   for (i=1; i<=n; ++i){
      for (j=i+1; j<=n; ++j)
	 S(j,i)=1.-2*x[MC_edge_index(i-1, j-1)];
      S(i,i)=1.;
   }
   NEWMAT::Jacobi(S,D,V);
   ...
   for (k=1; k<=n; ++k) {
      if ( D(k,k) >= 0. )
	 continue;
      // check that it's violated enough
      sum = 0;
      for (i=1; i<=n; ++i) {
	 sum += V(i,k) * V(i,k);
      }
      const double violation = D(k, k) * sum;
      if (-violation < minviol)
	 continue;
      index.push_back(k);
      viol.push_back(violation);
   }

   if (viol.size() > 0) {
      const int complete = MC_edges_in_complete(n);
      double* dels = new double[complete];
      int* colind = new int[complete];
      CoinIotaN(colind, complete, 0);

      CoinSort_2(viol.begin(), viol.end(), index.begin());
	 ...
	 for (i=1; i<=n; ++i) {
	    sum += V(i,k) * V(i,k);
	    for ( j=i+1; j<=n; ++j) {
	       const double coeff = V(i,k) * V(j,k);
	       sum += 2*coeff;
	       dels[MC_edge_index(i-1, j-1)] = coeff;
	    }
	 }
	 new_cuts.push_back(new MC_explicit_dense_cut(sum/4, complete, dels));
	 new_rows.push_back(new BCP_row(colind, colind + complete,
					dels, dels + complete,
					-1e40, sum/4));
      }
      delete[] colind;
