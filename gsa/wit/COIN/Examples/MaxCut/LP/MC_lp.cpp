...
static void
MC_lp_unique_cycle_cuts(BCP_vec<BCP_cut*>& new_cuts,
...
#ifdef MC_COMPILE_WITH_OSL
  if ((par.entry(MC_lp_par::LpSolver) & MC_UseOsl) != 0) {
     OsiOslSolverInterface* osl = new OsiOslSolverInterface;
     // EKKModel* model = osl->getModelPtr();
     //    ekk_messagesPrintOff(model, 1, 2999);
     return osl;
  }
#endif
...
#ifdef MC_COMPILE_WITH_OSL
	|| ((par.entry(MC_lp_par::LpSolver) & MC_UseOsl) != 0)
#endif
	)
      ) {
    started_exact = false;
...
#ifdef MC_COMPILE_WITH_OSL
  {
    OsiOslSolverInterface* osllp = dynamic_cast<OsiOslSolverInterface*>(lp);
    if (osllp) {
//     ekk_setRdweight(osllp->getModelPtr(), 1e-10);
       ekk_setCompressOff(osllp->getModelPtr());
    }
  }
#endif
}
			   ...
			   BCP_vec<BCP_row*>& new_rows)
{
  double max_lp_viol = 0.0;
  {
    ...
    // high minimum violation than the max violation of the last optimization.
  ...
  const bool tailoff_gap_rel =
    is_gap_tailoff_rel(par.entry(MC_lp_par::TailoffGapRelMinItcount)-1,
		       par.entry(MC_lp_par::TailoffGapRelMinImprovement),
		       objval);
  const bool tailoff_lb_abs =
    is_lb_tailoff_abs(par.entry(MC_lp_par::TailoffLbAbsMinItcount)-1,
		      par.entry(MC_lp_par::TailoffLbAbsMinImprovement),
		      objval);
  const bool tailoff_lb_rel =
    is_lb_tailoff_rel(par.entry(MC_lp_par::TailoffLbRelMinItcount)-1,
		      par.entry(MC_lp_par::TailoffLbRelMinImprovement),
		      objval);

  int i, j, cutnum = new_cuts.size();
  ...
  const bool mst =
    ...
    (gen_mst_cycle == MC_GenerateMstCycleCutsAsLastResort &&
     (cutnum == 0 || tailoff_gap_rel || tailoff_lb_abs || tailoff_lb_rel));
  ...
  if (mst) {
    ...
    MC_lp_unique_cycle_cuts(new_cuts, new_rows);
...
#ifdef MC_COMPILE_SDP_CUTS
  if (current_index() == 0 && par.entry(MC_lp_par::GenerateSDPCuts)) {
    MC_generate_SDP_cuts(mc, x,
			 par.entry(MC_lp_par::MaxSDPCutNum),
			 par.entry(MC_lp_par::MinSDPCutViolation),
			 new_cuts, new_rows);
    printf("MC:  SDP cuts: %i\n",
	   static_cast<int>(new_cuts.size()) - cutnum);
    cutnum = new_cuts.size();
  }
#endif
