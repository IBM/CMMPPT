...
MKC_lp::unpack_module_data(BCP_buffer& buf)
{
  ...
  const double currenttime = CoinCpuTime();
  MKC_enumerate_knapsacks(kss, enumerated_ks,
			  par.entry(MKC_lp_par::MaxEnumeratedSize),
			  par.entry(MKC_lp_par::MaxEnumeratedCount));
  printf("MKC: time spent on enumeration: %f\n", CoinCpuTime() - currenttime);

  start_time = BCP_time_since_epoch() / 1.0e6;
}
...
{
   switch (par.entry(MKC_lp_par::LpSolver)) {
   case 1: // Volume
			    ...
			    BCP_vec<BCP_col*>& new_cols)
{
   ...
   for (size_t i = 0; i < generated_vars.size(); ++i) {
...
MKC_lp::compute_lower_bound(const double old_lower_bound,
			    ...
			    const BCP_vec<BCP_cut*>& cuts)
{
   // Try to generate columns.
   const double detol = lpres.dualTolerance();
   // If no upper bound yet then go for negative rc, otherwise go for rc's
   // that are all high enough (if all rc is small then we can't go below the
   // upper bound anyway
   const double rc_bound = detol +
      (upper_bound() >= DBL_MAX/2 ?
       0.0 /* no upper bound yet => go for negative rc */ :
       CoinMax((lpres.objval()-upper_bound()) / kss.ks_num, 0.0));

   double lpobj =
      lpres.objval() +
      MKC_generate_variables(kss, ks_fixings, enumerated_ks,
			     lpres.pi(), rc_bound,
			     par.entry(MKC_lp_par::AddAllGeneratedVars),
