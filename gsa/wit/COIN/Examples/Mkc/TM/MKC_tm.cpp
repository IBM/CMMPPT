...
//        bool all_encountered_var =
//  	 lp_par.entry(MKC_lp_par::AddAllGeneratedVars);
      bool print_best_dj =
	 lp_par.entry(MKC_lp_par::PrintBestDj);

      BCP_vec<MKC_var*> seed_vars;

      MKC_generate_variables(kss, ks_fixings,
			     0, /* no enumerated KS's yet */
			     dual,
			     0.01, /*rc_bound*/
			     true, /*all_encountered_var*/
			     print_best_dj,
			     seed_vars);
      added_vars.reserve(added_vars.size() + seed_vars.size());
      for (size_t i = 0; i < seed_vars.size(); ++i) {
	 added_vars.push_back(seed_vars[i]);
      }
      delete[] ks_fixings;
      delete[] dual;
   }
...
MKC_tm::display_feasible_solution(const BCP_solution* soln)
{
   ...
   if (tm_par.entry(MKC_tm_par::TestFinalSolution)) {
      const int ncols = lp_solver->getNumCols();
	 ...
	 lp_solver->setColSetBounds(ind, ind+cnt, bnd);
	 delete[] bnd;
      }
      lp_solver->initialSolve();
      if (! lp_solver->isProvenOptimal())
	 abort();
      cnt = 0;
      for (i = vars.size() - 1; i >= 0; --i) {
	 MKC_var* var = vars[i];
   ...
   printf("     The total unused weight of the used slabs is: %f\n",
	  p_mps - a_mps);
   printf("\n");
   printf("MKC: Displaying solution =========== part 4 ==============\n\n");
   printf("MKC: Total running time: %f\n", endtime - starttime);
