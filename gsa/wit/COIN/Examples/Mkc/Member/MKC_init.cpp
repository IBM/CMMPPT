...
MKC_read_mps_file(MKC_tm& mkc)
{
...
#ifdef MKC_COMPILE_WITH_OSL
   mkc.lp_solver = new OsiOslSolverInterface;
#else
   mkc.lp_solver = new OsiClpSolverInterface;
#endif
   CoinMpsIO mpsio;
   ...
   if (mkc.tm_par.entry(MKC_tm_par::SolveLpForLB) ||
      ...
      mkc.lp_solver->loadProblem(*colMatrix,
   ...
   if (mkc.tm_par.entry(MKC_tm_par::SolveLpForLB)) {
      mkc.lp_solver->initialSolve();
      mkc.lp_par.set_entry(MKC_lp_par::LowerBound,
			   mkc.lp_solver->getObjValue());
   }
   ...
   if (! mkc.tm_par.entry(MKC_tm_par::TestFinalSolution)) {
	 ...
	 mkc.lp_solver->setColSetBounds(ind, ind+cnt, bnd);
	 delete[] bnd;
      }
      delete[] ind;
   }
