...
MKC_enumerate_knapsacks(MKC_knapsack_set& kss,
			...
			const int max_enumerated_count)
{
   ...
   for (i = 0; i < ks_num; ++i) {
	 ...
	 ks.enumerated = 
	    MKC_enumerate_one_ks(clr, entries, ks.size, enumerated,
				 ks_num, i, ks.cost, ks.capacity, flag,
				 w, max_enumerated_count);
      } else {
	 // Otherwise we have more than one colors
	 ks.enumerated = true;
	 for (j = 0; j < color_num && ks.enumerated; ++j) {
	    const int cbj = color_beg[j];
	    const int cbj1 = color_beg[j+1];
	    clr[0] = entries[cbj].color;
	    for (k = 0; k < j && ks.enumerated; ++k) {
		     ...
		     const int max_enumerated_count)
{
   ...
   for (l = 0; true; ++l) {
	 ...
	 *var_entries++ = ks_ind;
	 ...
	 if (++cnt == max_enumerated_count) {
	    purge_ptr_vector(enumerated);
	 }
